...
public class VMManager {
    ...
    /**
     * Validates that a VM ID contains only safe characters (alphanumeric).
     * This prevents command injection and path traversal attacks.
     *
     * @param vmID The VM ID to validate
     * @return true if the VM ID is safe, false otherwise
     */
    public static boolean isValidVMID(@NonNull String vmID) {
        if (vmID == null || vmID.isEmpty()) {
            return false;
        }
        
        // VM IDs should only contain alphanumeric characters (letters and digits)
        // This matches the format generated by startRamdomVMID()
        return vmID.matches("^[a-zA-Z0-9]+$");
    }

    /**
     * Sanitizes a VM ID by replacing any unsafe characters with safe alternatives.
     * If the VM ID is completely invalid, generates a new random VM ID.
     *
     * @param vmID The VM ID to sanitize
     * @return A sanitized VM ID containing only alphanumeric characters
     */
    @NonNull
    public static String sanitizeVMID(@NonNull String vmID) {
        if (vmID == null || vmID.isEmpty()) {
            return idGenerator();
        }
        
        // Remove all non-alphanumeric characters
        String sanitized = vmID.replaceAll("[^a-zA-Z0-9]", "");
        
        // If nothing remains after sanitization, generate a new ID
